...
public class KafkaProducerProxy extends ZmsProducerProxy {
    ...
    public KafkaProducerProxy(ZmsMetadata metadata, SLA sla, String instanceName) {
        super(metadata, sla, instanceName);
        this.instanceName = instanceName;
        start();
    }

    public KafkaProducerProxy(ZmsMetadata metadata, SLA sla, String instanceName, Properties properties) {
    ...
    @Override
    public void asyncSend(ZmsMessage zmsMessage, ZmsCallBack callBack) {
        ProducerRecord<String, byte[]> record = new ProducerRecord(metadata.getName(), zmsMessage.getKey(), zmsMessage.getPayload());
        long startTime = System.currentTimeMillis();
        zmsMetrics.msgBody().markSize(record.value().length);
