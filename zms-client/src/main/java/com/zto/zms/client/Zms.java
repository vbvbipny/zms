...
public class Zms extends AbstractZmsService {
    ...
    private void doSendOneway(String topic, SimpleMessage simpleMessage) {
        ...
        producer.oneway(ZmsMessageBuilder.newInstance().buildPayload(simpleMessage.getPayload()).buildKey(simpleMessage.getKey()).buildTags(simpleMessage.getTags()).buildDelayLevel(simpleMessage.getDelayLevel()).build());
    }
    ...
    private SendResponse doSendSync(String topic, SimpleMessage simpleMessage, Properties properties) {
        ...
        return producer.syncSend(ZmsMessageBuilder.newInstance().buildPayload(simpleMessage.getPayload()).buildKey(simpleMessage.getKey()).buildTags(simpleMessage.getTags()).buildDelayLevel(simpleMessage.getDelayLevel()).build());
    }
    ...
    private void doSendAsync(String topic, SimpleMessage simpleMessage, Properties properties, ZmsCallBack callBack) {
        ...
        Producer producer = ProducerFactory.getProducer(topic, properties);
        producer.asyncSend(ZmsMessageBuilder.newInstance().buildPayload(simpleMessage.getPayload()).buildKey(simpleMessage.getKey()).buildTags(simpleMessage.getTags()).buildDelayLevel(simpleMessage.getDelayLevel()).build(), callBack);
    }
