...
public class TopicMapperService {
     ...
     * 发送消息
     */
    public SendResponse sendMsg(SendTopicDTO dto) {
        //当前环境消息生产者
        Producer producer = multiEnvironmentProducerManager.getCurrentProducer(dto.getEnvId(), dto.getTopic());
        return producer.syncSend(ZmsMessageBuilder.newInstance()
                .buildPayload((dto.getMsg()).getBytes())
                .buildKey(dto.getKeys())
                .buildTags(dto.getTags())
                .build());
    }
