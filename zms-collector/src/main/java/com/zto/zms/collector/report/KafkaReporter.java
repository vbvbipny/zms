 ...
 */
@Component("kafka")
public class KafkaReporter implements IReporter {
    ...
    private final byte[] testMsg = "helloworld".getBytes();
    ...
    public void recordMaxOffset(ClusterMetadata cluster) {
        try {
            ...
            for (String topic : kafkaMetaManager.getClusterTopics(cluster)) {
                ...
                reporterHelper.saveStatisticTopicOffset(cluster.getClusterName(), topic, maxTopicOffset);

                String cacheKey = cluster.getClusterName() + "." + topic;
                topicMaxOffsets.put(cacheKey, maxTopicOffset);
                maxClusterOffset += maxTopicOffset;
            }
            reporterHelper.saveStatisticClusterOffset(cluster.getClusterName(), maxClusterOffset);
        } catch (Throwable e) {
    ...
    private void collectKafkaMetrics(ClusterMetadata cluster) {
        ...
        try {
            ...
            dataCarrier.produce(ZmsConst.Measurement.KAFKA_BROKER_INFO, kafkaBrokerLst);
            List<MetricsDo> kafkaTopicLst = metricsTransformService.transformKafkaTopic(kafkaMetrics);
            dataCarrier.produce(ZmsConst.Measurement.KAFKA_TOPIC_INFO, kafkaTopicLst);
            List<MetricsDo> kafkaEnvLst = metricsTransformService.transformKafkaEnv(kafkaMetrics);
            dataCarrier.produce(ZmsConst.Measurement.KAFKA_ENV_INFO, kafkaEnvLst);
            List<MetricsDo> clusterStrDataLst = metricsTransformService.transformStrDataKafkaCluster(kafkaMetrics);
            dataCarrier.produce(ZmsConst.Measurement.CLUSTER_STRING_INFO, clusterStrDataLst);
            List<MetricsDo> clusterDoubleDataLst = metricsTransformService.transformDoubleDataKafkaCluster(kafkaMetrics);
            dataCarrier.produce(ZmsConst.Measurement.CLUSTER_NUMBER_INFO, clusterDoubleDataLst);
        } catch (Exception e) {
    ...
    public void collectRtTime(ClusterMetadata cluster) {
        try {
            ...
            nodes.forEach(node -> {
                ...
                ProducerRecord record = new ProducerRecord(topic, testMsg);
    ...
    public void collectKafkaConsumerInfo(ClusterMetadata cluster) {
        try {
            ...
            dataCarrier.produce(ZmsConst.Measurement.KAFKA_CONSUMER_NUMBER_INFO, metricsDoList);
    ...
    public void topicTpsTop10() {
        try {
            ...
            dataCarrier.produce(ZmsConst.Measurement.TOPIC_TOP_INFO, tpsPOList);
        } catch (Throwable e) {
