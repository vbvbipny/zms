 ...
 */
@Component("rocketmq")
public class RocketMQReporter implements IReporter{
    ...
    @Autowired
    private ReporterHelper reporterHelper;

    @Autowired
    private ExtensionRepository extensionRepository;

    private static byte[] TEST_MSG = "helloworld".getBytes();
...
//                                    error code 206代表消费者不在线： CODE: 206  DESC: the consumer group[medivhOrderStatus] not online
            ...
            dataCarrier.produce(ZmsConst.Measurement.CLUSTER_NUMBER_INFO,mqClusterLst);
            //broker节点指标信息
            List<MetricsDo> mqBrokerLst = metricsTransformService.transformMqBroker(rocketmqStatus);
            dataCarrier.produce(ZmsConst.Measurement.MQ_BROKER_NUMBER_INFO,mqBrokerLst);
            List<MetricsDo> mqStrDataBrokerLst = metricsTransformService.transformStringDataMqBroker(rocketmqStatus);
            dataCarrier.produce(ZmsConst.Measurement.MQ_BROKER_STRING_INFO,mqStrDataBrokerLst);
            //topic tps信息
            List<MetricsDo> mqTopicLst = metricsTransformService.transformMqTopic(rocketmqStatus);
            dataCarrier.produce(ZmsConst.Measurement.MQ_TOPIC_INFO,mqTopicLst);
            ...
            dataCarrier.produce(ZmsConst.Measurement.MQ_CONSUMER_NUMBER_INFO,mqConsumerLst);
            List<MetricsDo> mqStrDataConsumerLst = metricsTransformService.transformStrDataMqConsumer(rocketmqStatus);
            dataCarrier.produce(ZmsConst.Measurement.MQ_CONSUMER_STRING_INFO,mqStrDataConsumerLst);
    ...
    public void collectRtTime(ClusterMetadata cluster) {
        ...
        for (Map.Entry<String, BrokerData> stringBrokerDataEntry : clusterInfo.getBrokerAddrTable().entrySet()) {
            ...
            try {
                sendResult = mqProducerManager.getMqProducer(cluster).send(new Message(brokerName, TEST_MSG));
        ...
        reporterHelper.saveRtMetrics(clusterRtTime);
    }
    ...
    public void recordMaxOffset(ClusterMetadata cluster) {
        ...
        try{
            ...
            for (String topic : mqMetaManager.getClusterTopics(cluster)) {
                ...
                reporterHelper.saveStatisticTopicOffset(clusterName, topic, maxTopicOffset);
                String cacheKey = clusterName + "." + topic;
                topicMaxOffsets.put(cacheKey, maxTopicOffset);
                maxClusterOffset += maxTopicOffset;
            }
            reporterHelper.saveStatisticClusterOffset(clusterName, maxClusterOffset);
    ...
    public void topicTpsTop10() {
        try {
            ...
            dataCarrier.produce(ZmsConst.Measurement.TOPIC_TOP_INFO, metricsDos);
        } catch (Throwable e) {
    ...
    public void consumerLatencyTop10() {
        try {
            ...
            dataCarrier.produce(ZmsConst.Measurement.CONSUMER_TOP_INFO, tpsPOList);
        } catch (Throwable e) {
